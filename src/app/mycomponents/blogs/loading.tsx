...
export default function LoadingBlogs(){
    ...
    const mapSkeletons = ()=>{
        ...
        return(
        Array.from([0,1,2]).map((blog,index)=>{
                    return(
                        <div  key={index} className=' font-mono bg-box border-none  flex flex-row flex-wrap gap-4 justify-between rounded-[5px] hover:scale-105 transition-all duration-200 w-[700px] box-border' >
                            ...
                            <div id='tags' className='flex gap-5 justify-start'>
                            {
                                ...
                                Array.from([0,1,2]).map((tag,index)=>{
                                    return(
                                        <Skeleton key={index}  className='  border-none w-[70px] h-[20px]  p-1 rounded-[5px] text-[12px]'></Skeleton>
                                    )
                                })
                            }
                            </div>
